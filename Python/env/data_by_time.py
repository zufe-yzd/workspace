            ...
            for i in range(0, len(order)):
                if orderData[i]["id"] == data[0]:
                    if data[0] not in did:
                        did.append(data[0])
                        print("Process: " + str(len(did)) + "/" + str(len(order)))
...
if __name__ == "__main__":
    ...
    perfile = int(len(orderData) / 10)

    for file in range(1,10):
        fw = open("../../Express/public/data/orders/data_by_1-50_order_pack" + str(file) + ".json", mode='w')  # 写入文件
    ...
    fw = open("../../Express/public/data/orders/data_by_1-50_order_pack10.json", mode='w')  # 写入文件
